...
pub fn run() {
    ...
    // Start the notification daemon once, before the reload loop, so it keeps owning the D-Bus name across bar config reloads (§8 "persists across reloads"). The popup surface itself is (re)established inside `run_once` on the driver thread — notification state lives in the daemon, so recreating it loses nothing.
...
fn setup_shell(config_path: PathBuf) {
    ...
    // The config the shell is currently running. A reload that fails to parse keeps this one rather than falling back to the starter bar, so a typo costs the user an error message, not their whole layout.
    let live = Rc::new(RefCell::new(Arc::clone(&config)));

    // One reconciliation, shared by every trigger: re-read the config and bring the surfaces in line with it. Driven by a config edit, by a monitor being plugged in or unplugged, and by `hogar-shell shell reload` — and it is the *same* pass at startup, so there is one description of what should be on screen rather than an opening path and a reloading path that can disagree.
    let reconcile = {
        ...
        move |content: Content| {
            let (config, content) = match Config::load(&config_path) {
                ...
                Err(e) => {
                    ...
                    // The file that would have changed what the surfaces draw does not parse, so nothing about them has changed: the last working config is reconciled for the surface *set* — a monitor may still have arrived — and every tree already drawing it is left alone.
            ...
            if content == Content::Rebuild {
                // Everything else that is on screen, in place and in the same pass: the panels the user opened over the chrome, and the notification popup that follows the focused screen. Each keeps its surface — and what the user was in the middle of — and takes the new config.
    ...
    // Reports what it did itself, at startup and at every reload alike — through tracing rather than `println!`, because this runs on the driver thread where a direct write to a pipe nobody is draining blocks forever. See `init_tracing`.
    surfaces.borrow_mut().reconcile(
        &config_path,
        &config,
        &platform_wayland::outputs(),
    ...
    // The config having changed, whoever noticed: the file watcher, `hogar-shell shell reload`, a keybind. The toast belongs here rather than in the reconcile, which also runs at startup — a toast saying the config was reloaded is only true of a reload.
...
/// Tells the user their edit didn't take, through the shell's own notification daemon so the message lands on screen rather than in a log they aren't reading. Falls back to stderr when the daemon isn't up yet.
...
/// The config-watch producer for `watch`: polls the config's mtimes (dependency-free, naturally debounced) and sends a tick on each change so the driver thread reconciles the surface set.
...
/// The mtimes the shell's layout depends on: `config.toml` and every `monitors/<output>/config.toml`. The per-monitor files are part of the same answer, so editing one has to trigger the same reload as editing the global file — a watcher that only knew about `config.toml` would leave a monitor override needing a restart.
