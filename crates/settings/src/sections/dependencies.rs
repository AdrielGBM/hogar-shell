...
//!
//! Readings, not fields — nothing here is a setting, so the form has no Save. It is the one page that answers "why is that module empty" without leaving the shell, which is why every row says what its absence costs rather than only whether it is there.
//!
//! Built in Rust rather than `.rsx` for the reason the applications list is: its rows are a list whose length the registry decides, and the value each shows arrives from a worker thread rather than from the config.
...
///
/// Probing is a process start and a bus round trip per row, so it cannot happen while this is being built: [`deps::report`] does it on a thread of its own and the page fills in when the answers arrive. Empty until then, which is honest — a row that has not been probed has nothing to say.
pub(crate) fn dependencies_section() -> Result<Box<dyn LayoutItem>, LayoutError> {
    ...
    // Keyed on the dependency, never on the position: the list is [`deps::ALL`] in order, and keying on an index would reuse one row's widgets for another the moment the report arrives and the list stops being empty.
...
mod tests {
    ...
    /// Every row reads as a sentence about *this* machine, whichever way the probe went — including the third answer, which is the one a naive `bool` would have turned into a false accusation.
    ///
    /// The section as a whole is built by `every_section_on_every_page_builds`, which walks the page table; what that cannot reach is a row for a presence this machine does not happen to produce.
    #[test]
