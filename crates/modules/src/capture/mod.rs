...
//!
//! The services below know how to capture pixels and drive a recorder; this is the layer that decides *which* pixels and what to do with them, so every entry point — a bar chip, `hogar-shell screenshot region`, the utilities panel — performs the same flow rather than each assembling its own request.
...
/// The screenshot `[screenshot]` describes for `target`, for a caller that already knows which pixels it wants (an IPC command naming an output).
...
/// A region the user draws. The still is discarded: a recording is live pixels, and the only thing the picker contributes is the rectangle.
pub fn record_region() {
    pick(move |picked: Picked| recorder::start(Scope::Area(picked.area)));
}

/// Stops a recording, or starts one of the whole screen. What the bar chip and the utilities toggle both do, so one press is always the useful one.
...
mod tests {
    ...
    /// Every flow resolves its own request from the config rather than hardcoding one, so a user who turned saving off gets a clipboard-only capture from the chip, the keybind and the panel alike.
    #[test]
