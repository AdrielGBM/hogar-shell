...
//!
//! The XDG rule, the `~` expansion, the `user-dirs.dirs` reading and the app-name scoping all live in `telar::paths`; what is left here is the part that is this shell's own — which directories it declares to the runtime through [`ShellPaths`], and the fallbacks it takes when the session provides none.
...
/// The directories this shell declares to Telar, so `telar::paths::*` and every widget behind it resolve the same places the shell writes to. Handed to the runner in place of the three stubs that used to answer `None` and leave every caller to work it out again.
...
/// Falls back to resolving the base directly, for the paths a surface asks for before the runner has installed anything — a bar builds its config path while the event loop is still being constructed.
...
mod tests {
    ...
    /// Every app directory is nested under the shell's own name, whether the runner installed a provider or the fallback resolved it — a bar that builds its path early must not write somewhere else than one that builds it late.
    #[test]
