...
/// Producer threads alive right now, by thread name, counted rather than flagged so a restart arriving before the outgoing thread has returned does not un-list the incoming one.
...
impl<T: Clone + Send + 'static> Store<T> {
    ...
    /// Applies `change` to the current value and fans the result out. Returns the new value so a caller can persist it without a second read racing another writer.
