...
//!
//! What `hyprctl dpms` does, asked of the compositor directly. The protocol is better than a dispatcher at the one thing that matters here: **it reports the mode back**. A `mode` event arrives when the control object is created and again after every change, whoever made it — so switching a screen off is verified rather than hoped for, and reading the current state needs no separate query.
//!
//! **A short-lived connection, not a watcher.** Destroying a `zwlr_output_power_v1` does not restore anything: the protocol says a mode change is effective immediately and says nothing about the object outliving it, which is the opposite of `zwlr-gamma-control` and the reason this needs no thread. Releasing the objects also releases the exclusivity the compositor grants with them, so a `wlopm` run afterwards is not refused.
...
/// How long to wait for every output to report the mode it was asked for. Long enough for a panel to come back from sleep, short enough that a caller waiting on a keybind is not left wondering.
const DEADLINE: Duration = Duration::from_millis(1500);

/// Whether the compositor can blank an output this way, asked over a connection of its own so it answers outside a running shell. `None` means no compositor could be reached.
...
///
/// `Err` distinguishes the two failures a caller can act on: a compositor without the protocol, and one that has it but would not do it — an output that cannot be blanked, or one another client holds.
...
/// Whether every output is currently on, or `None` where that cannot be asked. An output the compositor refuses to report is left out rather than counted as either.
...
impl Session {
    /// Takes a power control for every output. The compositor reports each one's current mode as soon as it exists, so the round trip that follows is also the reading.
...
impl Dispatch<ZwlrOutputPowerV1, ()> for State {
    fn event(
        ...
        match event {
            ...
            // The protocol asks that this object be dropped, and says nothing about the output's mode. So it stops counting toward "every output changed" rather than failing the whole request.
...
mod tests {
    ...
    /// A control the compositor disowned must not hold the whole request open, and must not be counted as having changed either — the two ways this could report a lie.
    #[test]
    ...
    /// Reading the mode, which needs the protocol but disturbs nothing — so it runs where the blanking test below would be too rude, and tells a missing protocol apart from a broken one.
    ///
    ...
    ///
    /// **It turns the screen off and back on.** There is no gentler way to prove a screen blanked: the mode is exactly what this reads back, and reading it without setting it proves only that the protocol answers.
    #[test]
