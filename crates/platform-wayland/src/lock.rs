...
//!
//! The session lives on the driver, alongside the layer surfaces, because it shares everything they use: one Wayland connection, one seat, one loop, and the same rsx handler path (a lock surface renders through [`LayerWindow`] exactly as a bar does).
...
/// The lock's state as the shell sees it, shared with the driver by atomics because the shell reads it from a UI handler while the driver writes it from a Wayland callback.
...
/// A live session lock. **Dropping it does not unlock** — an unlock has to be an explicit decision, since a handle going out of scope by accident would put the user's screen back on display.
pub struct LockHandle {
    shared: Arc<LockShared>,
}

impl LockHandle {
    /// Asks the compositor to unlock. Returns immediately; the driver performs `unlock_and_destroy` and tears the lock surfaces down on its next loop turn.
    pub fn unlock(&self) {
        self.shared.unlock.store(true, Ordering::Relaxed);
    }

    /// Whether the compositor has confirmed the lock. `false` between asking and being granted — the window in which the screen may still be showing the desktop, so nothing security-sensitive may act on it.
    pub fn is_locked(&self) -> bool {
        self.shared.locked.load(Ordering::Relaxed)
    }

    /// Whether the compositor refused the lock, or ended it itself. A refused lock leaves the session *unlocked* — the caller must treat it as a failure to lock, not as a lock it merely cannot draw on.
...
thread_local! {
    // A lock requested on the UI thread, mounted by the driver on its next turn — the same hand-off dynamic surfaces use, so a lock triggered from a click handler needs no second thread or connection.
    static LOCK_QUEUE: RefCell<Option<PendingLock>> = const { RefCell::new(None) };
}

/// Whether this compositor implements `ext-session-lock-v1` at all. Read before offering to lock: a shell that blanks its own screen with an overlay it cannot enforce is worse than one that says it cannot lock.
...
///
/// The factory is called once per output, immediately, and again for any output connected while the lock is up. Returns a handle whose [`LockHandle::is_locked`] turns true once the compositor confirms; a compositor that refuses reports through [`LockHandle::is_finished`] instead.
    ...
    F: Fn(Option<String>) -> A + 'static,
{
    ...
    LOCK_QUEUE.with(|queue| {
        ...
        // A request the driver never saw would otherwise leave its owner waiting on a lock that will never be granted or refused. Reported as finished, which is the answer "this did not happen".
...
/// The driver's live lock: the protocol object, how to build a surface for a new output, and which outputs are already covered.
...
fn start(driver: &mut Driver, pending: PendingLock, qh: &QueueHandle<Driver>) {
    ...
    let Some(manager) = driver.lock_manager.clone() else {
        // Not an error the shell can work around, and not one it should paper over with an overlay: without the protocol there is no way to stop the compositor rendering the desktop underneath.
...
/// Creates a lock surface for every output that has none — at lock time for all of them, and afterwards for a monitor plugged in while the screen is locked, which would otherwise show the compositor's fallback colour.
fn cover_outputs(driver: &mut Driver, compositor: &CompositorState, qh: &QueueHandle<Driver>) {
    // Checked on every loop turn for as long as the screen is locked, so the ordinary answer — "every output already has one" — costs a count rather than a collect.
    if driver
        .lock
        .as_ref()
...
fn create_surface(
    ...
    // A placeholder until the compositor's configure, which is authoritative: the lock surface must commit at exactly the size it acked, so nothing is rendered before that arrives.
    let logical = info
        .as_ref()
...
/// Ends the lock: `unlock_and_destroy` once the compositor confirmed it (`destroy` would be a protocol error), a plain `destroy` when it never did, then the surfaces. Flushed before returning, because the unlock is a request the compositor must have processed — an exit racing it would leave the session locked.
fn end(
    driver: &mut Driver,
    unlock: bool,
    conn: &Connection,
...
/// Drops the lock surface belonging to a disconnected output, so reconnecting the monitor gets a fresh one rather than being skipped as already covered.
...
impl Dispatch<ExtSessionLockSurfaceV1, ()> for Driver {
    fn event(
        ...
        // Acked before anything is drawn: committing a lock surface before its first ack is a protocol error, and the driver's render pass is gated on `configured`, which this is what sets.
